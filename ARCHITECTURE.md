...
| Модуль | Ответственность |
|--------|----------------|
| `app/` | Next.js App Router: layout, страницы (`page.tsx`) |
| `api/` | HTTP-клиент к backend API |
| `components/` | UI-компоненты (ReportCard, RunStatusBadge) |
...
### 4. Next.js (вместо SvelteKit / чистого React+Vite)

**Выбрано**: Next.js 15 (App Router) + React 19 + TailwindCSS v4.

**Альтернативы**:
- **React + Vite + React Router**: Легче, но требует отдельного nginx для сервинга в Docker, нет встроенного SSR при необходимости.
...
**Почему так**: Next.js даёт file-based routing (App Router), встроенный standalone-режим для Docker (без nginx), API rewrites для проксирования, и возможность SSR/SSG при будущем расширении. При этом страницы работают как `'use client'` — никакого SSR-оверхеда.
